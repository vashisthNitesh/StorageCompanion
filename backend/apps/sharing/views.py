        ...
        if node.spacebyte_hash:
            ...
            try:
                ...
                if not sb_client.is_configured:
                    return Response(
                        {
                            "error": "SpaceByte upstream storage is not configured (SPACEBYTE_ACCESS_TOKEN is missing or empty in .env).",
                ...
                return Response(
                    {"error": f"Failed to stream shared file from storage upstream: {str(e)}"},
                    status=status.HTTP_502_BAD_GATEWAY,
                )
        else:
            try:
